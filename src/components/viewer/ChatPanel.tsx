...
import { MessageSquare, StickyNote, RefreshCw, Loader2 } from "lucide-react";
import { ScrollArea } from "@/components/ui/scroll-area";
import { ChatInput } from "@/components/project/ChatInput";
import { SaveToNoteButton } from "@/components/chat/SaveToNoteButton";
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This action will delete all messages in this conversation. This cannot be undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
          ...
          {notebook && (
            <div className="mb-8 pb-6 border-b border-border/50">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-10 h-10 flex items-center justify-center">
                  {notebook.generation_status === 'generating' ? (
                    ...
                    <span className="text-4xl leading-none">{notebook.icon || '📝'}</span>
                  )}
                </div>
                <div>
              ...
              <div className="bg-muted/30 rounded-lg p-4">
                {notebook.generation_status === 'generating' ? (
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <p className="text-sm">AI is analyzing your source and generating a title and description...</p>
                  </div>
                ) : (
                  <div className="text-sm text-foreground leading-relaxed">
                    {notebook.description ? (
                      ...
                      <p className="text-muted-foreground">No description available for this notebook.</p>
                    )}
                  </div>
                )}
              </div>
...
function ChatMessageWithSave({ message, onCitationClick, notebookId }: { message: Message; onCitationClick?: (citation: any) => void; notebookId?: string }) {
  ...
  const handleCopy = () => {
    const contentStr = typeof message.content === 'string' ? message.content : message.content?.segments?.map(s => s.text).join('') || '';
    ...
    <div className="flex justify-start">
      <div className="w-full">
        ...
        >
          ...
          <SaveToNoteButton content={message.content} notebookId={notebookId} />
        </div>
      </div>
    </div>
  );
