...
export function NoteListItem({ note, onClick, onDelete }: NoteListItemProps) {
    ...
    if (note.source_type === 'ai_response') {
      ...
      try {
        ...
        if (parsed.segments && parsed.segments[0]) {
          return parsed.segments.slice(0, 3).map((s: any) => s.text).join(' ');
        }
      } catch (e) {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className="group flex items-start gap-2 p-2 rounded-lg hover:bg-studio-card/70 cursor-pointer transition-colors"
    >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground line-clamp-2 mt-0.5">{previewText}</p>
